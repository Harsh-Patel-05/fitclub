...
def addtocart(request, id):
    ...
    cartdata = order(i_id=item(id=pid), loginid=login(id=uid), o_quantity=1, total=price,oid=0,ostatus=0)
    cartdata.save()
    messages.success(request, 'Data Inserted Successfully.')
    return render(request, 'index.html')
...
def checkout(request):
    ...
    print(items)
    return render(request, 'checkout.html',{'products':fetchpro , 'cart':fetchcart,'login':nadd , 'tamount':items})
